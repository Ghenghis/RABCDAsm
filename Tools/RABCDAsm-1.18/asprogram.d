...
class ASProgramVisitor
{
    ...
    void readActionScript3(ubyte[] data)
    {
        auto abc = new ABCFile();
        abc.load(data);
        as.abcFiles ~= abc;
    }

    void readActionScript3WithoutLoad(ubyte[] data)
    {
        auto abc = new ABCFile();
        as.abcFiles ~= abc;
    }
    ...
    {
        string result;
        foreach (size_t i, abc; as.abcFiles)
            result ~= format("// ABC File %d:\n%s", i, abc.toString());
        return result;
    }
}
